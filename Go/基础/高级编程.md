...
defer函数帮助Parse从panic中恢复。在defer函数内部，**panic value被附加到错误信息中**；并用err变量接收错误信息，返回给调用者。我们也可以通过调用`runtime.Stack`往错误信息中添加完整的堆栈调用信息。
...
所以**通道是第一类对象：可以存储在变量中，作为函数的参数传递，从函数返回以及通过通道发送它们自身**。另外它们是类型化的，允许类型检查，比如尝试使用整数通道发送一个指针。
...
在 [channel_block2.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_14/channel_block2.go) 加入这种模式：
...
使用这些可以改进为版本 goroutine3.go，输出相同。
...
使用 **for-range** 语句来读取通道是更好的办法，因为这会**自动检测通道是否关闭**：
...
所谓Futures就是指：有时候在你**使用某一个值之前需要先对其进行计算**。这种情况下，你就可以在另一个处理器上进行该值的计算，到使用时，该值就已经计算完毕了。
...
使用带缓冲区的通道很容易实现这一点，其缓**冲区容量就是同时处理请求的最大数量**。
...
[server.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/server.go)
...
[client.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/client.go)
...
[dial.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/dial.go)
...
[socket.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/socket.go)
...
[twitter_status.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/twitter_status.go)
...
[simple_webserver.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/simple_webserver.go)
...
[robust_webserver.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/robust_webserver.go)
...
[wiki.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/wiki/wiki.go)
...
- 在 `main()` 中网页服务器用 `ListenAndServe` 启动并监听 8080 端口。需要先为紧接在 URL `localhost:8080/` 之后， 以`view`, `edit` 或 `save` 开头的 url 路径定义一些处理函数。在大多数网页服务器应用程序中，这形成了一系列 URL 路径到处理函数的映射，类似于 Ruby 和 Rails，Django 或 ASP.NET MVC 这样的 MVC 框架中的路由表。请求的 URL 与这些路径尝试匹配，较长的路径被优先匹配。如不与任何路径匹配，则调用 / 的处理程序。
...
[template_field.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/template_field.go)
...
[template_validation.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/template_validation.go)
...
运行 `Execute` 产生的结果来自模板的输出，它包含静态文本，以及被 `{{}}` 包裹的称之为*管道*的文本。例如，运行这段代码：
...
[template_ifelse.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_15/template_ifelse.go)
